...
class SoftmaxModel(Model):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE
        self.input_placeholder = tf.placeholder(tf.float32, shape=[self.config.batch_size, self.config.n_features],name="inputs")
        self.labels_placeholder = tf.placeholder(tf.float32, shape=[self.config.batch_size,self.config.n_classes], name="labels")
        ### END YOUR CODE
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("model"):
            ...
            b = tf.get_variable("b", shape=[self.config.n_classes],initializer=tf.random_normal_initializer(0, 0))
