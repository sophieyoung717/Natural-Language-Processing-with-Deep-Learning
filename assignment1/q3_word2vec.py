...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    dx = prob
    dx[target] -=1  ###(p-1)
    grad = dx.reshape((N,1))*r.reshape((1,D)) ###(p-1)*V_c 
    gradPred = (dx.reshape((1,N)).dot(outputVectors)).flatten() ###(p-1)*U_0
    ### END YOUR CODE
...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    predicted = 0.5 / C * np.sum(d, axis = 0)###The middle word

    cost, gradPred, gradOut = word2vecCostAndGradient(predicted, c_idx, outputVectors, dataset)

    gradIn = np.zeros(inputVectors.shape)
    for word in contextWords:
        gradIn[tokens[word]] += 0.5 / C * gradPred ###given contextword predict center word, whenever we have a context word, update center word correspondingly.
    ### END YOUR CODE
